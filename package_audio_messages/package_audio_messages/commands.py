...
################################################################### Base Functionality Code #########################################################################
    ...
    def loadInfo(self):
        ...
        self.default_image = Image.open(os.path.join(resources_path, 'default.png'))
        self.dancingFace = Image.open(os.path.join(resources_path, 'danceFace.png'))
        self.chargeFace = Image.open(os.path.join(resources_path, 'chargeFace.png'))
        ...
        self.artImages = {
            name: Image.open(os.path.join(resources_path, 'ArtworkInfo', f"{name.strip()}_info.png"))
            for name in list(self.artworks.keys())[:5]
        }

        # Load fun facts
        funfacts_path = os.path.join(resources_path, 'FunFacts', 'funfacts.txt')
        with open(funfacts_path, 'r') as factFile:
            self.facts = factFile.readlines()

        self.factImages = {
            i: Image.open(os.path.join(resources_path, 'FunFacts', f"image{i}.png"))
            for i in range(10)
        }
