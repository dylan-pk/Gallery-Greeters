...
//     RCLCPP_INFO(this->get_logger(), "Interrupt signal received: %s", interrupted_ ? "true" : "false");
// }

// void pController::goal_callback(const geometry_msgs::msg::PoseStamped msg)
// {
//     final_goal_orientation_ = msg.pose.orientation;
//     initial_orientation_ = odo_.pose.pose.orientation;
...
// // {
// //     RCLCPP_INFO(this->get_logger(), "🛑 Interrupted — stopping and resetting controller");

// //     geometry_msgs::msg::Twist stop_msg;
// //     cmd_vel_pub_->publish(stop_msg);

// //     // 🔧 FULL reset of navigation state
// //     path_.poses.clear();
// //     path_index_ = 0;
...
// //     {
// //         RCLCPP_WARN(this->get_logger(), "⚠️ path_index_ (%zu) >= path size (%zu)", path_index_, path_.poses.size());
// //         return;
// //     }
...
// // // double initial_yaw = getYaw(odo_.pose.pose.orientation);
// // // double goal_yaw = getYaw(final_goal_orientation_);
// // //     double angle_error = normalizeAngle(goal_yaw - initial_yaw);
// // // if(!corrected_){
// // // if(std::abs(angle_error) > M_1_PI){
// // //             geometry_msgs::msg::Twist rotate_msg;
// // //         rotate_msg.angular.z = Kp_angular * angle_error;
...
// void pController::move()
// {
//     // === 1. Exit early if path is empty ===
//     if (path_.poses.empty())
//         return;

//     // === 2. Interrupt check in sentry mode ===
//     if (sentry_mode_ && interrupted_)
//     {
//         RCLCPP_INFO(this->get_logger(), "🛑 Interrupted — stopping and resetting controller");
...
//     // === 3. Handle final goal orientation even if index is out of bounds ===
// if (path_index_ >= path_.poses.size() && !sentry_mode_)
// {
//     if (!has_final_orientation_)
//         return;

//     double final_dist = getDistanceError(path_.poses.back());
//     double yaw_now = getYaw(odo_.pose.pose.orientation);
//     double yaw_goal = getYaw(final_goal_orientation_);
//     double yaw_error = normalizeAngle(yaw_goal - yaw_now);

//     if (!final_pose_reached_ && final_dist < tolerance)
//     {
//         final_pose_reached_ = true;
//         RCLCPP_INFO(this->get_logger(), "📍 Final position reached.");
//     }

//     if (final_pose_reached_ && std::abs(yaw_error) > 0.1)
//     {
//         correcting_final_orientation_ = true;
...
//     // ✅ Whether corrected or already facing correctly, now publish
//     if (final_pose_reached_ && std::abs(yaw_error) <= 0.1)
//     {
//         geometry_msgs::msg::Twist stop_msg;
//         cmd_vel_pub_->publish(stop_msg);
...
//above is working for P controller
...
if (path_index_ >= path_.poses.size() && !sentry_mode_)
{
    ...
    if (angular_output > 0.5) angular_output = 0.5;
    if (angular_output < -0.5) angular_output = -0.5;
    ...
    if (std::abs(angular_error) > M_PI / 6)  // 15 degrees
        {
